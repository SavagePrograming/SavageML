...
from savageml.simulations import BinaryOrSimulation


def test_initial_state_initialized():
    simulation = BinaryOrSimulation()
...
def test_simulation_duplicates():
    simulation = BinaryOrSimulation(seed=1)
...
def test_simulation_shape_correct():
    ...
    simulation = BinaryOrSimulation(seed=1, shape=test_shape)
    result = simulation.step()
    assert result[0].shape == expected_test_shape
    assert result[1].shape == expected_out_shape


def test_or_simulation_all_true():
    test_shape = (2, 2)
    all_ones_seeds = 4
    simulation = BinaryOrSimulation(seed=all_ones_seeds, shape=test_shape)
    result = simulation.step()
    assert (result[0] == 1.0).all()
    assert (result[1] == 1.0).all()


def test_or_simulation_all_false():
    test_shape = (2, 2)
    all_zero_seeds = 36
    simulation = BinaryOrSimulation(seed=all_zero_seeds, shape=test_shape)
    result = simulation.step()
    assert (result[0] == 0.0).all()
    assert (result[1] == 0.0).all()


def test_or_simulation_50_50():
    test_shape = (2, 2)
    _50_50_seeds = 31
    simulation = BinaryOrSimulation(seed=_50_50_seeds, shape=test_shape)
    result = simulation.step()
...
def test_simulation_step_count():
    max_steps = 11
    simulation = BinaryOrSimulation(seed=1, max_steps=max_steps)
    count = 0
    for _ in simulation:
        count += 1
