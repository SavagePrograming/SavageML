...
from savageml.simulations import BinaryAndSimulation


def test_initial_state_initialized():
    simulation = BinaryAndSimulation()
...
def test_simulation_duplicates():
    simulation = BinaryAndSimulation(seed=1)
...
def test_and_simulation_shape_correct():
    test_shape = (5, 7)
    test_shape_batched = (1, 5, 7)
    expected_out_shape = (1, 1)
    simulation = BinaryAndSimulation(seed=1, shape=test_shape)
    result = simulation.step()
    assert result[0].shape == test_shape_batched
...
def test_and_simulation_all_true():
    test_shape = (2, 2)
    all_ones_seeds = 4
    simulation = BinaryAndSimulation(seed=all_ones_seeds, shape=test_shape)
    result = simulation.step()
...
def test_and_simulation_all_false():
    test_shape = (2, 2)
    all_zero_seeds = 36
    simulation = BinaryAndSimulation(seed=all_zero_seeds, shape=test_shape)
    result = simulation.step()
...
def test_and_simulation_50_50():
    test_shape = (2, 2)
    _50_50_seeds = 31
    simulation = BinaryAndSimulation(seed=_50_50_seeds, shape=test_shape)
    result = simulation.step()
...
def test_simulation_step_count():
    max_steps = 11
    simulation = BinaryAndSimulation(seed=1, max_steps=max_steps)
    count = 0
    for _ in simulation:
        count += 1
