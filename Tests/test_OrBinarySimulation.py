...
from savageml.simulations import OrBinarySimulation


def test_initial_state_initialized():
    simulation = OrBinarySimulation()
...
def test_simulation_duplicates():
    simulation = OrBinarySimulation(seed=1)
...
def test_or_simulation_all_true():
    test_shape = (2, 2)
    all_ones_seeds = 4
    simulation = OrBinarySimulation(seed=all_ones_seeds, shape=test_shape)
    result = simulation.step()
...
def test_or_simulation_all_false():
    test_shape = (2, 2)
    all_zero_seeds = 36
    simulation = OrBinarySimulation(seed=all_zero_seeds, shape=test_shape)
    result = simulation.step()
...
def test_or_simulation_50_50():
    test_shape = (2, 2)
    _50_50_seeds = 31
    simulation = OrBinarySimulation(seed=_50_50_seeds, shape=test_shape)
    result = simulation.step()
...
def test_simulation_step_count():
    max_steps = 11
    simulation = OrBinarySimulation(seed=1, max_steps=max_steps)
    count = 0
    for _ in simulation:
        count += 1
